...
/**
 * 篇章完形填空API
 * 基础路径: /api/7_passage_cloze
 */
export const passageClozeApi = {
  /**
   * 健康检查
  ...
  /**
   * 获取题库信息
   * GET /api/7_passage_cloze/info
   */
  getBankInfo: () => {
    console.log('【API】调用 getBankInfo');
    return request({
      url: '/7_passage_cloze/info',
      method: 'get'
    }).catch(error => {
  ...
  /**
   * 获取篇章题目
   * GET /api/7_passage_cloze/passage
  ...
  submitPassage: ({ passageId, questionIds, answers, timeSpent = 0 }) => {
    ...
    return request({
      url: '/7_passage_cloze/passage/submit',
      method: 'post',
  ...
  getPassageDetails: (passageId) => {
    ...
    return request({
      url: `/7_passage_cloze/passage/${passageId}/details`,
      method: 'get'
    }).catch(error => {
...
export default passageClozeApi;