...
import {
    ...
    Refresh, Add, Shuffle, TrendingUp, AccessTime
...
const SimpleWordBook = ({ G_json, onClose }) => {
    ...
    const handleDeleteWord = async (word) => {
        ...
        try {
            ...
            setWords(original);
            setSelectedWords(prev => { const next = new Set(prev); next.delete(word); return next; });
            await deleteWordFromServer(word, getToken, G_json);
    ...
    return (
        <div style={{ position: 'fixed', top: '10%', left: '2.5%', width: '95vw', height: '90vh', backgroundColor: 'white', boxShadow: '0 0 30px rgba(0,0,0,0.4)', borderRadius: '12px', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            <Button onClick={onClose} sx={{ position: 'absolute', top: '10px', right: '10px', minWidth: 'auto' }}>✕</Button>
            <div style={{ flex: 1, overflow: 'auto', padding: '20px' }}>
                <Container sx={{ py: 3 }}>
                    <Box sx={{ mb: 3 }}>
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                            <Typography variant="h5" sx={{ fontWeight: 'bold' }}>{G_json}.json ({stats.total}词)</Typography>
                    ...
                    <Paper sx={{ p: 2, mb: 2 }}>
                        <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', alignItems: 'center' }}>
                            <TextField placeholder="搜索..." size="small" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} sx={{ width: 200 }} InputProps={{ startAdornment: <InputAdornment position="start"><Search fontSize="small" /></InputAdornment> }} />
                    ...
                    <Paper sx={{ overflow: 'hidden' }}>
                        <TableContainer sx={{ maxHeight: 500 }}>
                            <Table stickyHeader size="small">
                                <TableHead>
                                    <TableRow>
                                        <TableCell padding="checkbox"><Checkbox checked={paginatedWords.length > 0 && selectedWords.size === paginatedWords.length} indeterminate={selectedWords.size > 0 && selectedWords.size < paginatedWords.length} onChange={() => setSelectedWords(selectedWords.size === paginatedWords.length ? new Set() : new Set(paginatedWords.map(item => item.word)))} /></TableCell>
                                        <TableCell><TableSortLabel active={sortField === 'word'} direction={sortDirection} onClick={() => handleSort('word')}>单词</TableSortLabel></TableCell>
                                        <TableCell>中文</TableCell>
                                        <TableCell><TableSortLabel active={sortField === 'mastery'} onClick={() => handleSort('mastery')}>掌握</TableSortLabel></TableCell>
                                        <TableCell><TableSortLabel active={sortField === 'tests'} onClick={() => handleSort('tests')}>测试次数</TableSortLabel></TableCell>
                                        <TableCell><TableSortLabel active={sortField === 'extraction'} onClick={() => handleSort('extraction')}>抽取</TableSortLabel></TableCell>
                                        <TableCell><TableSortLabel active={sortField === 'accuracy'} onClick={() => handleSort('accuracy')}>正确率</TableSortLabel></TableCell>
                                        <TableCell><TableSortLabel active={sortField === 'lastReview'} onClick={() => handleSort('lastReview')}>最近学习</TableSortLabel></TableCell>
                                        <TableCell align="center">操作</TableCell>
                                    </TableRow>
                                </TableHead>
                                <TableBody>
                                    {paginatedWords.length ? paginatedWords.map(item => (
                                        <TableRow key={item.word} hover selected={item.isSelected}>
                                            ...
                                            <TableCell><Typography sx={{ fontWeight: 'medium' }}>{item.word}</Typography></TableCell>
                                            <TableCell>{item.chinese}</TableCell>
                                            <TableCell><Tooltip title={`测试: ${item.correct_count}对/${item.wrong_count}错`}><Chip label={item.masteryInfo.label} size="small" color={item.masteryInfo.color} variant="outlined" icon={<Help fontSize="small" />} /></Tooltip></TableCell>
                                            <TableCell>{item.total_tests}</TableCell>
                                            <TableCell>{item.extraction_count}</TableCell>
                                            <TableCell><Typography sx={{ color: item.accuracy >= 80 ? 'success.main' : item.accuracy >= 60 ? 'info.main' : item.accuracy >= 40 ? 'warning.main' : 'error.main' }}>{item.accuracy}%</Typography></TableCell>
                                            <TableCell>
                                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                                                    <AccessTime fontSize="small" color="action" />
                                                    <Typography variant="body2">{item.timeAgo}</Typography>
                                                </Box>
                                            </TableCell>
                                            <TableCell align="center">
                                                <Box sx={{ display: 'flex', gap: 0.5 }}>
                                                    ...
                                                    <IconButton size="small" onClick={() => handleDeleteWord(item.word)} color="error"><Delete fontSize="small" /></IconButton>
                                                </Box>
                                            </TableCell>
                                        </TableRow>
                                    )) : <TableRow><TableCell colSpan={9} align="center"><Alert severity="info">{Object.keys(words).length ? '没有匹配的单词' : '单词本为空'}</Alert></TableCell></TableRow>}
                                </TableBody>
                            </Table>
                        </TableContainer>
                        <TablePagination rowsPerPageOptions={[5, 10, 25, 50]} component="div" count={processedWords.length} rowsPerPage={rowsPerPage} page={page} onPageChange={(e, p) => setPage(p)} onRowsPerPageChange={(e) => { setRowsPerPage(parseInt(e.target.value, 10)); setPage(0); }} />
                    </Paper>

                    {selectedWords.size > 0 && <Paper sx={{ p: 2, mt: 2, bgcolor: 'action.hover' }}><Box sx={{ display: 'flex', justifyContent: 'space-between' }}><Typography>已选 {selectedWords.size} 词</Typography><Box sx={{ display: 'flex', gap: 1 }}><Button size="small" startIcon={<CheckCircle />} onClick={() => handleBatchMark(true)}>正确</Button><Button size="small" startIcon={<Cancel />} onClick={() => handleBatchMark(false)}>错误</Button><Button size="small" color="error" startIcon={<Delete />} onClick={() => setShowDeleteDialog(true)}>删除</Button></Box></Box></Paper>}
                            ...
                            <List>
                                {extractedWords.map((item, index) => (
                                    <React.Fragment key={item.word}>
                                        <ListItem>
                                            <ListItemText primary={<Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}><Typography>{index + 1}. {item.word}</Typography><IconButton size="small" onClick={() => window.speechSynthesis.speak(new SpeechSynthesisUtterance(item.word))}><VolumeUp fontSize="small" /></IconButton></Box>} secondary={`${item.chinese} · 测试: ${item.correct_count || 0}对/${item.wrong_count || 0}错`} />
                                        </ListItem>
                    ...
                    {/* 删除确认对话框 */}
                    <Dialog open={showDeleteDialog} onClose={() => setShowDeleteDialog(false)}><DialogTitle>确认删除</DialogTitle><DialogContent><Typography>确定删除选中的 {selectedWords.size} 个单词？此操作不可撤销。</Typography></DialogContent><DialogActions><Button onClick={() => setShowDeleteDialog(false)}>取消</Button><Button onClick={handleDeleteSelected} color="error" variant="contained">确认删除</Button></DialogActions></Dialog>

                    {/* 重置统计对话框 */}
                    <Dialog open={showResetStatsDialog} onClose={() => setShowResetStatsDialog(false)}><DialogTitle>重置统计</DialogTitle><DialogContent><Typography>重置 "{wordToReset}" 的统计信息？</Typography></DialogContent><DialogActions><Button onClick={() => setShowResetStatsDialog(false)}>取消</Button><Button onClick={() => handleResetStats(wordToReset)} color="warning" variant="contained">确认重置</Button></DialogActions></Dialog>

                    <Snackbar open={snackbar.open} autoHideDuration={3000} onClose={() => setSnackbar({ ...snackbar, open: false })}><MuiAlert severity={snackbar.severity}>{snackbar.message}</MuiAlert></Snackbar>
