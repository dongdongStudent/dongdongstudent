...
router.get("/translate_word", async (req, res) => {
  ...
  if (word.includes(" ") && /[\.\?\!]/.test(word)) {
    ...
    console.log("4444", word, translation);
    // 这里可以处理translation，例如发送给客户端
    res.json({
      flag: 1,
      message: "翻译成功",
...
router.get("/translate_mp3", async (req, res) => {
  ...
  // 1. 解析token
  let word = req.headers.content.trim(); // 去除前后空格
  // 处理word格式,去除word结尾后面所有的字符
  word = word.replace(/[.,?!;:]+$/, ""); // 去除结尾的标点符号
  // 将"替换为'
  word = word.replace(/"/g, "'");
  // 消去中间的?,末尾的不用消去
  word = word.replace(/\?/, "");
  ...
  const handleDownload = async () => {
    try {
      ...
      let path_file = "";
      if (word.length > 256) {
        word_1 = generateUniqueName(word, 20); // 生成唯一文件名
        path_file = path.join(
          __dirname,
          "../..",
          isSentence
      ...
      // 先判断文件是否存在
      await res.download(path_file, `${word}.mp3`, async (err) => {
        if (err) {
          await youdao_audio(word_or_sentence, word); // 调用youdao_audio函数获取音频
          // 在下载一次
          res.download(path_file, `${word}.mp3`, (err) => {
            if (err) {
              console.error("再次下载失败:", err);
              res.status(404).json({ message: "音频文件未找到" });
            }
          });
        } else {
          // 其他处理逻辑
...
router.get("/translate_chinese_mp3", async (req, res) => {
  const handleDownload = async (word, path_file) => {
    console.log("寻找音频文件", path_file);
    try {
      if (fs.existsSync(path_file)) {
        // 文件存在，直接下载
        res.download(path_file, (err) => {
          console.log("下载音频文件:", path_file);
          // 运行到这里说明下载成功，返回文件
        ...
        console.log("文件不存在，请求百度翻译");
        let state = await textToAudio(word);
        console.log("请求百度翻译成功:", path_file);

        // 将 Buffer 数据写入文件
        fs.writeFile(path_file, state, (err) => {
          if (err) {
            console.error("写入文件失败:", err);
            res.status(500).json({ message: "文件写入失败" });
          } else {
  ...
  let word = req.query.word.trim(); // 从查询参数获取
  // 如果req.headers.content为空，则返回错误信息
  if (!word) {
    return res.json({
      flag: 1,
  ...
  const isSentence = word.includes(",") || /[\.\?\!]/.test(word); //
  let word_name = word.replace(/[, ]+/g, ""); // 同时消去逗号和空格

  // 如果word_name的长度超过100，取开头10个和结尾10个字符
  // 将中文字符转换为哈希值
  const generateFileName = (word_name) => {
    // 生成一个哈希值作为文件名
    return crypto.createHash("md5").update(word_name).digest("hex");
  };

  if (word_name.length > 100) {
    word_name = word_name.slice(0, 10) + "___" + word_name.slice(-10);
  } else if (word_name.length > 120) {
    word_name = word_name.slice(0, 120);
  }
  ...
  await handleDownload(word, path_file);
});
