...
//   console.log('12333',filePath)
  ...
  if (!fs.existsSync(dirPath)) {
    ...
    console.log(`[获取数据] 用户: ${username_2}, 类型: ${wordType}, 创建目录: ${dirPath}`);
  }

  // 2. 检查文件是否存在，不存在则创建
  if (!fs.existsSync(filePath)) {
    console.log(`[获取数据] 用户: ${username_2}, 类型: ${wordType}, 文件不存在，创建新文件`);
  ...
  console.log(`[获取数据] 用户: ${username_2}, 类型: ${wordType}, 实时读取成功`);
  return data;
};
...
router.get("/get_words/:type?", async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.log(`[获取数据] 用户: ${username_2}, 类型: ${wordType}, 读取失败，创建新文件成功`);
    ...
    try {
      ...
      if (type === "add" || type === "delete") {
        ...
        if (type === "add") {
          ...
          console.log(`[添加成功] 用户: ${username_2}, 类型: ${wordType}, 单词: ${word}, 翻译: ${translation}`);
        ...
        if (type === "delete") {
          ...
          await writeWordData(username_2, wordType, dataToSave);
          console.log(`[删除成功] 用户: ${username_2}, 类型: ${wordType}, 单词: ${word}`);
