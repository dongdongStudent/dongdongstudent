...
const ListeningFinalMaster = () => {
  ...
  const toggleRepeatMode = () => {
    ...
    if (newMode && currentWord) {
      speakWordWithRepeat(currentWord.english, 2);
    }
  };
    ...
    return (
      <Container maxWidth="sm" sx={{ mt: 3, p: 2 }}>
        ...
        <Paper sx={{ p: 3, borderRadius: 4 }}>
          ...
          {/* 分数 */}
          <Typography variant="h1" sx={{ textAlign: 'center', color: accuracy >= 60 ? '#4caf50' : '#f44336', fontWeight: 'bold', my: 2 }}>
          ...
          <Grid container spacing={2} sx={{ mb: 3 }}>
            <Grid item xs={4}>
              <Paper sx={{ p: 2, textAlign: 'center', bgcolor: '#e8f5e8', borderRadius: 2 }}>
                <CheckCircle sx={{ color: '#4caf50', fontSize: 32 }} />
                <Typography variant="h5" sx={{ fontWeight: 'bold', color: '#2e7d32' }}>{stats.correct}</Typography>
                <Typography variant="caption" color="text.secondary">答对</Typography>
              </Paper>
            </Grid>
            <Grid item xs={4}>
              <Paper sx={{ p: 2, textAlign: 'center', bgcolor: '#fff3e0', borderRadius: 2 }}>
                <Typography variant="h5" sx={{ fontWeight: 'bold', color: '#ed6c02' }}>{formatTime(timeSpent)}</Typography>
                <Typography variant="caption" color="text.secondary">总用时</Typography>
                <Typography variant="caption" display="block" color="text.secondary">
                  {totalQuestions > 0 ? `${(timeSpent / totalQuestions).toFixed(1)}秒/题` : ''}
                </Typography>
              </Paper>
            </Grid>
            <Grid item xs={4}>
              <Paper sx={{ p: 2, textAlign: 'center', bgcolor: '#ffebee', borderRadius: 2 }}>
                ...
                <Typography variant="h5" sx={{ fontWeight: 'bold', color: '#c62828' }}>{stats.wrong}</Typography>
                <Typography variant="caption" color="text.secondary">答错</Typography>
              </Paper>
            </Grid>
          </Grid>
          ...
          {/* 错题列表 */}
          {wrongPool.length > 0 && (
            <Paper sx={{ p: 2, mb: 3, bgcolor: '#fff5f5', maxHeight: 200, overflow: 'auto', borderRadius: 2 }}>
              <Typography variant="subtitle2" sx={{ fontWeight: 'bold', mb: 1, color: '#f44336', display: 'flex', alignItems: 'center', gap: 1 }}>
                <Cancel fontSize="small" /> 需要复习的错题 ({wrongPool.length})
              </Typography>
              <Divider sx={{ mb: 1 }} />
          ...
          {/* 学习建议 */}
          <Box sx={{ mt: 2, p: 1.5, bgcolor: '#f5f5f5', borderRadius: 2, textAlign: 'center' }}>
            <Typography variant="caption" color="text.secondary">
    ...
    <Container maxWidth="lg" sx={{ mt: 1, px: 2, pb: 4 }} onClick={handleNextAction}>
      ...
      <Paper sx={{ 
        ...
        <Typography variant="body2">
          ...
          {repeatMode && repeatCount > 0 && ` (播放${repeatCount}/2次)`}
        </Typography>
      </Paper>
      ...
      <Card 
        ...
        onClick={(e) => { e.stopPropagation(); currentWord && speakWordWithRepeat(currentWord.english, 2); }}
      >
        {selectedId ? (
      ...
      <Box sx={{ display: 'flex', justifyContent: 'center', width: '100%' }}>
        ...
        >
          {options.map((opt, i) => {
            ...
            return (
              <Grid item xs={6} sm={3} md={3} key={i} sx={{ display: 'flex', justifyContent: 'center' }}>
                <Button 
                  fullWidth 
                  variant="contained" 
                  ...
                  sx={{ 
                    ...
                    textAlign: 'center',
                    borderRadius: 3,
                    boxShadow: selectedId ? 0 : 2
                  }}
                >
                  <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 1 }}>
                    {opt.chinese}
                    {selectedId && isCorrectOption && <CheckCircle sx={{ fontSize: 18 }} />}
                    {selectedId && isSelected && !isCorrectOption && <Cancel sx={{ fontSize: 18 }} />}
                  </Box>
                </Button>
              </Grid>
            );
