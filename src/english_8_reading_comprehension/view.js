...
const ReadingMasterView = ({
  ...
  // 优先使用本地状态，忽略父组件的expandedRow
  const activeExpandedRow = localExpandedRow;
  
  const handleExpandClick = (passageId) => {
    const newExpandedState = activeExpandedRow === passageId ? null : passageId;
    console.log('展开/收起:', passageId, newExpandedState);
            ...
            <TableHead>
              <TableRow>
                {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
                  <TableCell key={i}>
                    <Skeleton variant="text" width={i === 1 ? 50 : 80} />
                  </TableCell>
                ))}
              </TableRow>
            </TableHead>
          ...
          <TableBody>
            {paginatedPassages.length > 0 ? (
              paginatedPassages.map((passage) => {
                ...
                const isExpanded = activeExpandedRow === passage.id;
                const difficulty = getDifficultyLabel(passage.difficulty);
                const extractCount = passageStat?.extract_count || 0;
                
                // 计算掌握度（基于正确率）
                let masteryLevel = 0;
                if (passageStat?.history && passageStat.history.length > 0) {
                  const latestHistory = passageStat.history[passageStat.history.length - 1];
                  masteryLevel = Math.round(latestHistory.accuracy * 100);
                }
                  ...
                  <React.Fragment key={passage.id}>
                    ...
                    >
                      <TableCell padding="checkbox">
                        <IconButton
                          size="small"
                          onClick={() => handleExpandClick(passage.id)}
                                      ...
                                      <Grid item xs={6} md={3}>
                                        <Box sx={{ textAlign: 'center' }}>
                                          <Typography variant="h6" color="success.main">
                                            {passageStat.history?.reduce((sum, h) => sum + h.correctCount, 0) || 0}
                                          </Typography>
                                          <Typography variant="caption" color="text.secondary">
                                            总正确数
                                          </Typography>
                                      ...
                                      <Grid item xs={6} md={3}>
                                        <Box sx={{ textAlign: 'center' }}>
                                          <Typography variant="h6" color="error.main">
                                            {passageStat.history?.reduce((sum, h) => sum + (h.totalQuestions - h.correctCount), 0) || 0}
                                          </Typography>
                                          <Typography variant="caption" color="text.secondary">
                                            总错误数
                                          </Typography>
                                      ...
                                      <Card key={idx} variant="outlined" sx={{ mb: 2 }}>
                                        <CardContent>
                                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, flexWrap: 'wrap', mb: 2 }}>
                                            <Chip
                                              size="small"
                                              icon={h.accuracy >= 0.8 ? <CheckCircleIcon /> : h.accuracy >= 0.6 ? <HistoryIcon /> : <CancelIcon />}
                                              label={`正确率: ${Math.round(h.accuracy * 100)}%`}
                                              color={h.accuracy >= 0.8 ? 'success' : h.accuracy >= 0.6 ? 'warning' : 'error'}
                                            />
                                            <Typography variant="body2" color="text.secondary">
                                              时间: {formatDate(h.date)}
                                            </Typography>
                                            <Typography variant="body2" color="text.secondary">
                                              用时: {h.timeSpent}秒
                                            </Typography>
                                            <Typography variant="body2" color="text.secondary">
                                              {h.correctCount}/{h.totalQuestions} 正确
                                            </Typography>
                                          </Box>
                                              ...
                                              {h.results.map((r, rIdx) => (
                                                <Grid item xs={12} sm={6} key={rIdx}>
                                                  <Box sx={{ 
                                                    ...
                                                    p: 1,
                                                    borderRadius: 1,
                                                    bgcolor: r.isCorrect ? 'rgba(76, 175, 80, 0.1)' : 'rgba(244, 67, 54, 0.1)'
                                                  }}>
                                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                                      ...
                                                      <Box>
                                                        <Typography variant="body2" sx={{ color: r.isCorrect ? '#2e7d32' : '#c62828' }}>
                                                          您的答案: {r.userAnswer || '无'}
                                                        </Typography>
                                                        {!r.isCorrect && (
...
export default ReadingMasterView;